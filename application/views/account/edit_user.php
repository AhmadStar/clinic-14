...
<div class="col col-md-8 well well-md">
    ...
    <div style="margin-bottom: 20px;color: black;">
        <?php trP('EditUserInformationfor')?> :<br />
        <center>
            <?php echo $user->username;?>
        </center><input type="hidden" name='username' id='username' value="<?php echo set_value('username',$user->username);?>" />
    </div>
    <?php echo (!empty($error) ? $error : '' ); ?>

    <legend>-
        <?php trP('PersonalInformation')?>:</legend>
    <div>
        <div class="form-group">
            ...
            <div class="col-md-12" style="margin-bottom:10px;">
                <label class="radio-inline"><input type="radio" name='gender' value="1" title='Male' <?php echo isset($_POST['gender'])?($this->input->post('gender')?'checked':''):($user->gender?'checked':'');?> />
                    <?php trP('Male')?></label>
                <label class="radio-inline"><input type="radio" name='gender' value="0" title='Female' <?php echo isset($_POST['gender'])?($this->input->post('gender')?'':'checked'):($user->gender?'':'checked');?> />
    ...
    <legend>-
        <?php trP('AccountSettings')?>:</legend>
    <div>
        <?php if($this->bitauth->is_admin()){?>
        <div class="form-group">
            <div class="col-md-3">
                <?php echo form_label(form_checkbox('active',1,isset($_POST['active'])?($_POST['active']?TRUE:FALSE):($user->active?TRUE:FALSE),"id='active'").trp('Active'),'active',$checkbox_conf_array);?>
            </div>
            <div class="col-md-3">
                <?php echo form_label(form_checkbox('enabled',1,isset($_POST['enabled'])?($_POST['enabled']?TRUE:FALSE):($user->enabled?TRUE:FALSE),"id='enabled'").trp('Enable'),'enabled',$checkbox_conf_array);?>
            </div>
            <div class="col-md-6">
                <?php echo form_label(form_checkbox('password_never_expires',1,isset($_POST['password_never_expires'])?($_POST['password_never_expires']?TRUE:FALSE):($user->password_never_expires?TRUE:FALSE),"id='password_never_expires'").trP('PasswordNeverExpires'),'password_never_expires',$checkbox_conf_array);?>
            </div>
        </div>
        <div class="clearfix"></div>
        <div class="form-group" title="Select all groups you would like the user to be in.">
            <div class="col-md-12"><label for="groups[]">
                    <?php trP('groups')?>:</label>
                <?php echo form_multiselect('groups[]', $groups, set_value('groups[]', $user->groups),"class='form-control'");?>
            </div>
        </div>
        <?php }if(!$this->bitauth->is_admin()){?>
    ...
    <legend>-
        <?php trP('Disease')?>:</legend>
    <div>
        <div class="form-group">
            <div class="col-md-12"><textarea name="disease" id="disease" class="form-control" rows="10"><?php echo set_value('disease',$user->disease);?></textarea></div>
        </div>
    </div>
